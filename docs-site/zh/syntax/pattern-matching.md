...
不带类型名的花括号模式 —— `{field: subpattern, ...}` —— 按字段名匹配 `Struct` 值。花括号里写出 `field: subpattern`,只有当字段存在且子模式也匹配时,该字段才算匹配成功:
...
裸字段模式并不要求列出所有字段——只要被列出的字段都能匹配即可,其余字段被忽略。这使得它非常适合"按需提取几个字段"的场景。它是下文 `TypeName { ... }` 形式的无名版本。
...
这里 `Point { x: 0, y: 0 }` 用字面量精确匹配原点,而 `Point { x: px, y: py }` 把字段绑定到变量。和裸字段模式一样,Struct 模式只检查你列出的字段,未列出的字段不参与匹配。
...
模式匹配把"分支"与"解构"合二为一。通过字面量、变量、通配符、or、Vec、字段、Struct、Variant 这一套模式原语,再配合守卫,你几乎可以用一种声明式的方式描述"我期待什么样的数据,以及见到它之后该做什么"。当你发现自己写了长长的 `if` 链来检查值的类型和结构时,通常意味着该把它们重写成一个 `match` 了。
